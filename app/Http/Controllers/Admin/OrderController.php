...
class OrderController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.statOrder', $data);
    }

    public function statOrder(Request $request)
    {
        ...
        if ($pro == "all") {
            $ann = Orders::leftJoin('orders_details', 'orders_details.order_id', '=', 'orders.id')
                ->whereBetween('orders.created_at', [$ssd, $eed])
                ->groupBy('orders_details.product_id')
                ->select('orders_details.product_name', 'orders_details.product_id', DB::raw('count(*) as total'))
                ->get();
            $count = count($ann);
            return response()->json(array(
                "status" => "success",
            ...
            $productName=Product::find($pro);
            $ann = Orders::leftJoin('orders_details', 'orders_details.order_id', '=', 'orders.id')
                ->where('orders_details.product_id', $pro)
                ->whereBetween('orders.created_at', [$ssd, $eed])->get();
            $count = count($ann);
            return response()->json(array(
                "result" => $ann,
                "status" => "success",
